...
def get_realtime_top_etfs(api_key):
    ...
    # 第二階段：拿著這 15 檔名單，一檔一檔向富果要「當下即時報價」(完全符合基本用戶免費權限！)
...
class Alpha_Engine:
    ...
    def calculate_historical_vwap(ticker, window=20):
        try:
            ...
            ma20 = data['Close'].rolling(window=window).mean().iloc[-1]
            vwap20 = vwap.iloc[-1]
            return ma20, vwap20
        except:
            return None, None
...
# --- 戰情室網頁介面實作 ---
st.title("🦅 將軍天眼 5.0：混合雙雷達即時版")
st.caption(f"📡 數據源：富果 Fugle 毫秒級單股狙擊 API | 系統每 30 秒自動刷新 | 同步時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for i, (idx, row) in enumerate(top_12.iterrows()):
        with cols[i % 4]:
            with st.container(border=True):
                ...
                if row['Change'] > 0:
                    st.markdown(f":red[現價: {row['Price']:.2f} ▲]")
                elif row['Change'] < 0:
                    st.markdown(f":green[現價: {row['Price']:.2f} ▼]")
                else:
                    st.markdown(f"現價: {row['Price']:.2f} ▬")
...
if not full_list.empty:
    ...
    for idx, row in top_8_df.iterrows():
        ...
        if vwap20 is not None and ma20 is not None:
            ...
            if price < ma20 and price >= vwap20:
                signal, intensity = "🟢 假跌破", "🔥 強烈買進"
            elif price < vwap20:
                signal, intensity = "🔴 趨勢破壞", "❄️ 嚴禁接刀"
            elif bias > 5.0:
                signal, intensity = "🔴 正乖離過大", "⚠️ 逢高減碼"
            elif price >= ma20 and price >= vwap20:
                signal, intensity = "🟢 穩定多頭", "🛡️ 抱緊持股"
            else:
                signal, intensity = "🟠 震盪整理", "⏳ 觀望等待"
        else:
            vwap20 = np.nan
            bias = np.nan
            signal, intensity = "⚪ 數據建置中", "📡 新兵入陣 (無月線)"
            
        decision_data.append({
            "代號": code, "名稱": name,
            "即時現價": price, "歷史主力成本(VWAP)": vwap20, "成本乖離%": bias,
    ...
    if decision_data:
        ...
        styled_df = df_final.style.format(
            formatter={"即時現價": "{:.2f}", "歷史主力成本(VWAP)": "{:.2f}", "成本乖離%": "{:.2f}"},
            na_rep="-"
        ).map(
    ...
    st.info("💡 **昊天戰略提醒**：雷達已鎖定盤中最強 8 檔精銳。即時報價每 30 秒自動刷新，與您的凱基系統完美連動！")